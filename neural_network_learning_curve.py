...
def visualise_training_curves(train_mean, train_std, val_mean, val_std, train_sizes):
    plt.plot(train_sizes, train_mean, color='blue', marker='o', markersize='5', label='Training accuracy')
    plt.fill_between(train_sizes, train_mean + train_std, train_mean - train_std, alpha=0.15, color='blue')

    plt.plot(train_sizes, val_mean, color='black', marker='s', linestyle='--', markersize='5', label='Validation accuracy')
    plt.fill_between(train_sizes, val_mean + val_std, train_mean - val_std, alpha=0.15, color='black')
...
def main():
    ...
    for i in range(len(dataset_sizes)):
        ...
        df = df.iloc[:int(dataset_sizes[i]/0.4)]
        ...
        lc_train_means.append(np.mean(training_accuracies))
        lc_train_std.append(np.std(training_accuracies))
        lc_val_means.append(np.mean(validation_accuracies))
        lc_val_std.append(np.std(validation_accuracies))
    ...
    visualise_training_curves(np.array(lc_train_means), np.array(lc_train_std), np.array(lc_val_means), np.array(lc_val_std), dataset_sizes)
